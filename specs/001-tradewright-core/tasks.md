...
- [ ] T093 Joint economy model (plan "Known Design Gaps" tuning precondition): simulation script over launch content checking SC-006 (≤60% self-sufficiency) and SC-007 (arbitrage profitability ±50% income parity) jointly, implementing the behavior model and healthy-world/equivalent-investment definitions of research R16 with all simulation parameters (actor counts, warm-up, window, seeds, decision cadence) as named constants in the test; tune `packages/content/data/*.json` until green, in `packages/content/tests/economy-model.test.ts`
