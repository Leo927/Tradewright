...
- [ ] T050 [US0] Settings screen v1 per design artifact: language list by endonym from `text/locales.json` (`status: shipped` locales only — validation locales never appear in the player-facing list), selection dispatches SetDisplayLocale optimistically and re-renders the whole GUI from catalogs immediately (local-immediate class, protocol Part V; SC-012) in `apps/client/src/screens/settings.tsx`
