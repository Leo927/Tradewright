...
- [ ] T005 [P] Scaffold `apps/client/` React 18 + Vite PWA: `package.json` (deps: `react`, `react-dom`, `@tradewright/contract`; dev: `vite`, `vite-plugin-pwa`), `vite.config.ts`, `index.html`, `src/main.tsx`, portrait-phone base styles
...
- [ ] T007 [P] Configure dependency-cruiser one-way boundary rules (client→contract only; engine→contract+content; contract→nothing; content→nothing; no engine→client edges) in `.dependency-cruiser.cjs`, wired into `npm run check` (constitution Gate 3)
...
- [ ] T070 [P] [US4] Playwright flow (quickstart US4): place sell order (escrow visible) → crossing buy fills with tax disclosed → goods/coin land correctly → other settlement's book never shows the listing → NPC drift check in `apps/client/tests/e2e/market.spec.ts`
